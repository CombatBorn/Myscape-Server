...
object PawnPathAction {
    ...
    fun getDifferenceY(one:Tile, two:Tile): Int  {
        return abs(one.z - two.z)
    }

    fun getDifferenceX(one:Tile, two:Tile): Int  {
        return abs(one.x - two.x)
    }

    private fun inAttackRange(start:Tile, end:Tile, npcSize:Int, range:Int):Boolean {

        var xOffset = 0
        var yOffset = 0

        if (start.z < end.z) {
            val offset = getDifferenceY(start, end)

            yOffset = if (offset > npcSize) {
                npcSize - 1
            } else {
                offset
            }
        ...
        if (start.x < end.x ) {
            ...
            val offset = getDifferenceX(start, end)


            xOffset = if (offset > npcSize) {
                npcSize - 1
            } else {
                offset
            }
        ...
        var newTile = Tile(start.x + xOffset, start.z + yOffset, 0)


        if (newTile.isWithinRadius(end, range)) {
            return true;
        }
    ...
    suspend fun walkTo(it: QueueTask, pawn: Pawn, target: Pawn, interactionRange: Int, lineOfSight: Boolean): Boolean {
        ...
        val vision = pawn.world.collision.raycast(sourceTile, targetTile, true)
        val overlap = overlap(sourceTile, sourceSize, targetTile, targetSize)

        while (!pawn.tile.sameAs(route.tail)) {

            if (inAttackRange(pawn.tile, target.tile, sourceSize,interactionRange) && vision && !overlap) {
                pawn.stopMovement()
                return true
            }
