    ...
    private fun write(buf: GamePacketBuilder, blockType: UpdateBlockType, forceFace: Tile?) {
        ...
        when (blockType) {
            UpdateBlockType.PUBLIC_CHAT -> {
                ...
                val chatLength = chatMessage.text.length
            ...
            UpdateBlockType.APPLY_TINT -> {
                val structure = blocks.updateBlocks[blockType]!!.values
                buf.put(structure[0].type, structure[0].order, structure[0].transformation, other.blockBuffer.maskStartCycle) // recolourStartCycle
                buf.put(structure[1].type, structure[1].order, structure[1].transformation, other.blockBuffer.maskEndCycle) // recolourEndCycle
                buf.put(structure[2].type, structure[2].order, structure[2].transformation, other.blockBuffer.maskHue) // recolourHue
                buf.put(structure[3].type, structure[3].order, structure[3].transformation, other.blockBuffer.maskSaturation) // recolourSaturation
                buf.put(structure[4].type, structure[4].order, structure[4].transformation, other.blockBuffer.maskLuminance) // recolourLuminance
                buf.put(structure[5].type, structure[5].order, structure[5].transformation, other.blockBuffer.maskRecolourAmount) // recolourAmount
            }
