...
suspend fun combat(it: QueueTask) {
    ...
    while (npc.canEngageCombat(target)) {
        ...
        if (canAttack && npcDelayBool) {
            val windMagicAttack = windMagicAttack(npc, target)
            (target as Player).message("test1")
            if (!windMagicAttack.landed) {
                it.wait(2)
...
fun attemptFreeze(target: Pawn) {
    if (world.random(10) == 1) {
        target.freeze(6)
        target.graphic(56)
        when (world.random(5)) {
            1 -> {target.forceChat("This wind is crazy!")}
            2 -> {target.forceChat("woooooaaahhhh!")}
            3 -> {target.forceChat("That one almost blew me away..")}
            4 -> {target.forceChat("That was a lot of built up air..")}
            5 -> {target.forceChat("I can't move!")}
        }
    }
}
...
fun windMagicAttack(npc: Npc, target: Pawn) : PawnHit {
    val projectile = npc.createProjectile(
            target, gfx = 91, startHeight = 40, endHeight = 36, delay = 41, angle = 15, steepness = 240
    )
    npc.prepareAttack(CombatClass.MAGIC, CombatStyle.MAGIC, AttackStyle.ACCURATE)
    npc.animate(4652)
    world.spawn(projectile)
    val hit = npc.dealHit(
            target = target,
            Air_giant_plugin.WindRangedAttackFormula(13.0, 0),
            delay = RangedCombatStrategy.getHitDelay(npc.getFrontFacingTile(target), target.getCentreTile()) - 1
    )
    if (hit.landed == false){
...
class WindRangedAttackFormula(val maxHit: Double, val minHit: Int = 0) : CombatFormula {
