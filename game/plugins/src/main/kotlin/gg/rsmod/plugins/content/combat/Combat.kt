...
//    fun overlapping(ox1:Int, oy1:Int, owidth:Int, olength:Int, ox2:Int, oy2:Int, owidth2: Int, olength2:Int ) : Boolean {
//        val a = Box(ox1, oy1, owidth - 1, olength - 1)
//        val b = Box(ox2, oy2, owidth2 - 1, olength2 - 1)
//
...
//    }
    ...
    suspend fun moveToAttackRangeCustom(it: QueueTask, pawn: Pawn, target: Pawn, distance: Int, projectile: Boolean):Boolean {
    val world = pawn.world
    val start = pawn.tile
    val end = target.tile
...
//    val withinRange = touching && world.collision.raycast(start, end, projectile = projectile)
        ...
        println("Are overlapping ${areOverlapping(start.x, start.z, srcSize, srcSize, end.x, end.z, dstSize, dstSize)}")
        println("Are bordering ${areBordering(start.x, start.z, srcSize, srcSize, end.x, end.z, dstSize, dstSize)}")

//        println("Range Boolean ${withinRange} Touching Boolean ${touching}")

    return PawnPathAction.walkTo(it, pawn, target, interactionRange = distance, lineOfSight = false)
    ...
    fun canEngage(pawn: Pawn, target: Pawn): Boolean {
        ...
        val maxDistance = when {
            ...
            if ((!pawn.tile.isWithinRadius(target.tile, maxDistance) && (pawn.attr[JAD_ATTR] == null))) {
                return false
            }
