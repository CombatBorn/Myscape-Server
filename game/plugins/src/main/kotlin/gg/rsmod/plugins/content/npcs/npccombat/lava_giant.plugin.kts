...
set_combat_def(Npcs.LAVA_GIANT) {
    configs {
        attackSpeed = 2
        respawnDelay = 50
...
suspend fun combat(it: QueueTask) {
    ...
    while (npc.canEngageCombat(target)) {
        ...
        if (canAttack && npcDelayBool) {
            val lavaMagicAttack = lavaMagicAttack(npc, target)
            (target as Player).message("test1")
            if (!lavaMagicAttack.landed) {
                it.wait(2)
...
fun attemptLavaBomb(target: Pawn) {
    if (world.random(10) == 1) {
        target.doubleHit(5,5)
        target.doubleHit(5,5)
        target.removeCombatTarget()
        target.graphic(287)
        when (world.random(5)) {
            1 -> {target.forceChat("My prayer!")}
            2 -> {target.forceChat("I can't focus..")}
            3 -> {target.forceChat("HOT!!!")}
            4 -> {target.forceChat("IT BURNS!..")}
            5 -> {target.forceChat("What Happened??")}
        }
    }
}
...
fun lavaMagicAttack(npc: Npc, target: Pawn) : PawnHit {
    val projectile = npc.createProjectile(
            target, gfx = 130, startHeight = 40, endHeight = 36, delay = 41, angle = 15, steepness = 240
    )
    npc.prepareAttack(CombatClass.MAGIC, CombatStyle.MAGIC, AttackStyle.ACCURATE)
    npc.animate(4652)
    world.spawn(projectile)
    val hit = npc.dealHit(
            target = target,
            Lava_giant_plugin.LavaRangedAttackFormula(13.0, 0),
            delay = RangedCombatStrategy.getHitDelay(npc.getFrontFacingTile(target), target.getCentreTile()) - 1
    )
    if (hit.landed == false){
...
class LavaRangedAttackFormula(val maxHit: Double, val minHit: Int = 0) : CombatFormula {
