...
fun Pawn.dealHit(target: Pawn, formula: CombatFormula, delay: Int, onHit: (PawnHit) -> Unit = {}): PawnHit {
    ...
    return dealHit(target, maxHit, -1, landHit, delay, onHit)
}

fun Pawn.dealHit(target: Pawn, maxHit: Int, setHit: Int, landHit: Boolean, delay: Int, onHit: (PawnHit) -> Unit = {}): PawnHit {
    val hit = if (landHit) {
        if (setHit != -1){
            target.hit(setHit, delay = delay)
        }else{
...
suspend fun Pawn.moveToAttackRangeCustom(it: QueueTask, target: Pawn, distance: Int, projectile: Boolean): Boolean = Combat.moveToAttackRangeCustom(it, this, target, distance, projectile)
